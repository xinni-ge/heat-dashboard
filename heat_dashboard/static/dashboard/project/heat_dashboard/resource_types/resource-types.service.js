...
(function() {
   ...
   * is documented below.
   */
  function resourceTypeService($filter,
                        $location,
                        heat,
                        userSession,
                        settings,
    ...
    return {
      ...
      getResourceTypePromise: getResourceTypePromise,
      getResourceTypesPromise: getResourceTypesPromise,
    };
    ...
    function getDetailsPath(item) {
      return detailRoute + 'OS::Heat::ResourceType/' + item.resource_type;
    }
    ...
    function getResourceTypesPromise(params) {
      ...
      function getResourceTypes(userSession) {
        return heat.getResourceTypes(params).then(modifyResponse);
      }

      function modifyResponse(response) {
        return {data: {items: response.data.items.map(modifyResourceType)}};

        function modifyResourceType(resourceType) {
          return resourceType;
        }
      }
    }
